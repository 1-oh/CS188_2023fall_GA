...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        if not (len(self.keys()) == 0 or self.total() == 0):
            sum_of_all = self.total()
            for key in self.keys():
                origin_value = self.get(key)
                self[key] = origin_value / sum_of_all
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for old_position in self.allPositions:
            ...
            for new_position in newPosDist.keys():
                newBeliefs[new_position] += newPosDist[new_position] * self.beliefs[old_position]
                # 后验信念 正比于 观察似然 * 先验信念
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        num_possible_pos = len(possiblePos)
        num = self.numParticles
        
        for index in range(num):
            self.particles.append(possiblePos[index % num_possible_pos])
        "*** END YOUR CODE HERE ***"
